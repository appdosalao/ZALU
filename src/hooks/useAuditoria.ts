...
export function useAuditoria() {
  ...
  const executarAgora = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        toast.error('Erro ao executar auditoria no servidor. Usando modo local.');
        setRelatorioBackend(null);
        return false;
      }
      setRelatorioBackend(data as any);
  ...
  const resolverProblemasSelecionados = useCallback(async (selecionados: ProblemaAuditoria[]) => {
    if (!user) return false;
    ...
    try {
      ...
      for (const p of selecionados) {
        const { error } = await supabase
          ...
          .eq('resolvido', false)
          .eq('tipo', p.tipo)
          .eq('entidade', p.entidade)
          .eq('entidade_id', p.entidadeId);
        if (!error) atualizados++;
      }
      if (atualizados > 0) {
        toast.success(`${atualizados} problema(s) marcados como resolvidos`);
        carregarHistorico();
      } else {
        toast.info('Nenhum problema correspondente encontrado no histórico');
      }
      return true;
    } catch (e) {
      toast.error('Erro ao resolver problemas selecionados');
      return false;
    }
  }, [user, carregarHistorico]);

  const reabrirProblemasSelecionados = useCallback(async (selecionados: ProblemaAuditoria[]) => {
    if (!user) return false;
    if (selecionados.length === 0) return true;
    try {
      let atualizados = 0;
      ...
      if (atualizados > 0) {
        toast.success(`${atualizados} problema(s) reabertos`);
        carregarHistorico();
      } else {
        toast.info('Nenhum problema reaberto');
      }
      return true;
    } catch (e) {
      toast.error('Erro ao reabrir problemas selecionados');
      return false;
    }
  }, [user, carregarHistorico]);

  // Realtime: atualizar histórico quando relatórios/problemas mudarem
  useEffect(() => {
    if (!user) return;
